-- 1) Add team mode + banner to tournaments
...
CREATE TABLE IF NOT EXISTS public.tournament_teams (
  ...
  CONSTRAINT fk_team_tournament FOREIGN KEY (tournament_id) REFERENCES public.tournaments(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS public.tournament_team_members (
  ...
  CONSTRAINT fk_member_team FOREIGN KEY (team_id) REFERENCES public.tournament_teams(id) ON DELETE CASCADE
);

-- 4) Trigger function to keep updated_at
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_tournament_teams_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_tournaments_updated_at'
  ) THEN
...
-- 5) Capacity + counters management
...
BEGIN
  ...
  -- Recompute and set tournaments.current_participants from members count for that tournament
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_manage_team_membership'
  ) THEN
...
-- 6) Whenever a team is created, auto-create captain membership
CREATE OR REPLACE FUNCTION public.add_captain_membership()
RETURNS trigger AS $$
BEGIN
  -- Ensure team current_members starts at 1 for captain
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_add_captain_membership'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Anyone can view tournament teams'
  ) THEN
    CREATE POLICY "Anyone can view tournament teams" ON public.tournament_teams FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Captain or admin can update team'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Captain or admin can delete team'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can create team as captain'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Anyone can view team members'
  ) THEN
    CREATE POLICY "Anyone can view team members" ON public.tournament_team_members FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can join a team as themselves'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can leave their team'
  ) THEN
...
-- 8) Helper function to enforce team_size on team creation
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_enforce_team_size_on_create'
  ) THEN
